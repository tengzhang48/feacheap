subroutine compute_static_step
    ...
    integer :: iteration
    integer :: i
    integer :: total_steps, total_cutbacks
    ...
    write(6,'(A)')    ' ================================================'
    write(6,'(A)')    '  feacheap: starting static analysis'
    write(6,'(A,I8)') '  Elements: ', n_elements
    write(6,'(A,I8)') '  Nodes:    ', n_nodes
    write(6,'(A)')    ' ================================================'
    ...
    if (solvertype==1) then                        ! Direct solver
        ...
        do while (continue_timesteps)
            ...
            if (fail) then                          ! Force a timestep cutback if stiffness computation fails
                converged=.false.
                iteration = 0
                total_cutbacks = total_cutbacks + 1
                write(6,'(A,I5,A,E10.3,A)') &
                    '  Step ', current_step_number, &
                    '  dt=', DTIME, &
                    '  ASSEMBLY FAILED - cutting back'
            ...
            do iteration = 1,max_newton_iterations
                ...
                if (fail) then
                    write(6,'(A,I5,A,I3,A)') &
                        ...
                        '  iter ', iteration, &
                        '  ASSEMBLY FAILED'
                    exit                         ! Force a cutback if stiffness computation fails
                endif
            ...
            if (.not.converged) then
                ...
                write(6,'(A,I5,A,E10.3,A,I3,A)') &
                    '  Step ', current_step_number, &
                    '  dt=', DTIME, &
                    '  NOT converged after ', max_newton_iterations, ' iters - cutting back'
            endif
        ...
        do while (continue_timesteps)
            ...
            if (fail) then                          ! Force a timestep cutback if stiffness computation fails
                ...
                write(6,'(A,I5,A,E10.3,A)') &
                    '  Step ', current_step_number, &
                    '  dt=', DTIME, &
                    '  CG ASSEMBLY FAILED - cutting back'
                call compute_static_time_increment(iteration,converged,continue_timesteps, &
                    activatestateprint,activateuserprint,new_time_increment)
                dof_increment = 0.d0
                cycle
            ...
            if (nonlinear) then                          ! Nonlinear problem - activate Newton iterations
                do iteration = 1,max_newton_iterations
                    ...
                    if (converged) then
                        write(6,'(A,I5,A,E10.3,A,I3,A)') &
                            '  Step ', current_step_number, &
                            '  dt=', DTIME, &
                            '  converged in ', iteration, ' iterations (CG)'
                        exit
                    endif
                    call solve_conjugategradient(fail)
                end do
                ...
                if (.not.converged) then
                    ...
                    write(6,'(A,I5,A,E10.3,A,I3,A)') &
                        '  Step ', current_step_number, &
                        '  dt=', DTIME, &
                        '  NOT converged after ', max_newton_iterations, ' iters (CG)'
                endif
            endif
    ...
    write(6,'(A)')    ' ================================================'
    write(6,'(A)')    '  Static analysis complete'
    write(6,'(A,I8)') '  Steps completed: ', total_steps
    write(6,'(A,I8)') '  Cutbacks:        ', total_cutbacks
    write(6,'(A,E12.4)') '  Final time:      ', TIME
    write(6,'(A)')    ' ================================================'
...
subroutine compute_static_time_increment(iteration,converged,continue_timesteps, &
    ...
    if (.not.converged) then
        ...
        if (new_time_increment<timestep_min) then
            ...
            write(6,'(A)') ' *** FATAL: timestep cut below minimum ***'
            write(6,'(A,E12.4)') '   Min timestep: ', timestep_min
